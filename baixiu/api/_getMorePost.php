...
// 1 获取必要的参数
$currentPage = $_POST["currentPage"]; //当前要加载第几页
$pageSize = $_POST["pageSize"]; //每页加载多少条数据
// 计算出要从哪里开始获取
...
/* 
        3 还得计算出所有的文章总共刻意获取多少次, 返回给前台让其判断加载更多是否应该显示给用户
    */
    if(!empty($_POST["categoryId"])) {
        $sqlCount = "SELECT count(id) as postCount FROM posts WHERE category_id = {$categoryId}";
     
    } else {
        $sqlCount = "SELECT count(id) as postCount FROM posts ";
    }
...
// 3.3 因为查询结构是二维数组, 需要从二维数组中获取总共能获取的次数
// print_r($countArr);
...
/* 
        ...
        {
            "code": 请求的状态 - 约定如果是某些特定的数字, 代表不同的状态. 比如: 1 代表成功, 0 代表失败
            "msg": 想要返回给前台的一些信息
            "data": 返回给前台的数据
        }
    */
